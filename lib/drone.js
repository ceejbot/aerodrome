...
Drone.prototype.handleNav = function(navdata)
{
	...
	this.handleFlyState(navdata.demo.flyState);
	this.handleBatteryState(!!navdata.droneState.lowBattery);

	// snag the whole thing, I guess
	this.droneState   = navdata.droneState;
...
var flyingPat = /^FLYING_/;
Drone.prototype.handleFlyState = function(state)
{
	if (state == this.flyState)
		return;

	this.isFlying = flyingPat.test(test);
	this.flyState = state;
	this.emit('log', 'fly state is now ' + this.flyState);
};

Drone.prototype.handleBatteryState = function(newstate)
{
	if (newstate == this.lowBattery)
		return;

	if (!newstate)
	{
		this.emit('log', 'battery low');
		this.emit('battery', 'low');
	}

	this.lowBattery = newstate;
};
